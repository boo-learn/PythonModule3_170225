...
def add_word(english_word: str, russian_translation: str, db_path: Path = Path(DATABASE_NAME)) -> str:
    ...
    if english_word == "" or russian_translation == "":
        raise ValueError("ПустойВвод")  #("Английское слово и русский перевод не могут быть пустыми.")
    try:
        with Connect(db_path) as cursor:
            cursor.execute(sql_insert, (english_word.lower().strip(), russian_translation.lower().strip()))
        return"🆕✅ Слово добавлено в словарь."
    except sqlite3.Error as e:
        # print(f"Дідько!!! Не удалось добавить слово в базу данных: ")
        raise ValueError(e)
